...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image mb-2" src=${product.image}></img>
      ...
      <h4 class="heading">Price: $ ${product.price}</h4>
      <h5>Rating : ${product.rating.rate}</h5>
      <h6>${product.rating.count} people rated this product</h6>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-info mt-2">add to cart</button>
      <button id="details-btn" class="btn btn-danger mt-2">Details</button></div>
      `;
...
const updatePrice = (id, value) => {
  ...
  const total = (convertedOldPrice + convertPrice).toFixed(2);  // used toFixed method for showing 2 decimal points
